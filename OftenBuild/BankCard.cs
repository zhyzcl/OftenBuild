...
namespace AppPub
{
    /// <summary>银行卡类</summary>
    public class BankCard
    {
        #region 数组形式存储银行BIN号

        /// <summary>银行BIN号数组</summary>
        public readonly static int[] BankBin =
        {
            102033,
            103000,
            185720,
        ...
        /// <summary>发卡行.卡种名称数组</summary>
        public static readonly string[] BankName = {
            "广东发展银行.广发理财通",
            "农业银行.金穗借记卡",
            "昆明农联社.金碧卡",
            "中国光大银行.阳光爱心卡",
        ...
        /// <summary>二分法查找数组索引</summary>
        /// <param name="srcArray">BIN数组</param>
        /// <param name="des"></param>
        /// <returns>二分法查找数组索引</returns>
        public static int BinarySearch(IReadOnlyList<int> srcArray, long des)
        {
            int low = 0;
        ...
        /// <summary>银行卡号校验</summary>
        /// <param name="bankno">银行卡号</param>
        /// <param name="err">发生错误则返回错误信息</param>
        /// <returns>银行卡号校验</returns>
        public static bool BankCardCheck(string bankno, ref string err)
        {
            if (bankno.Length < 16 || bankno.Length > 19)
            {
                err = "银行卡号长度必须在16到19之间！";
                return false;
            }
            string num = @"^[0-9]*[1-9][0-9]*$";
            if (!System.Text.RegularExpressions.Regex.IsMatch(bankno, num))
            {
                err = "银行卡号必须全为数字！";
                return false;
            }
            ...
            int lastNum = Convert.ToInt32(bankno.Substring(bankno.Length - 1, 1));//取出最后一位
            string firstNum = bankno.Substring(0, bankno.Length - 1);//前15或18位
            int luhm = GetNumLuhm(firstNum);//计算Luhm值
            if (lastNum == luhm)
            {
                return true;
            }
        ...
        /// <summary>获取数字字符串的Luhm值</summary>
        /// <param name="firstNum">数字字符串</param>
        /// <returns>获取数字字符串的Luhm值</returns>
        public static int GetNumLuhm(string firstNum)
        {
            List<int> newArr = new List<int>();
            for (int i = firstNum.Length - 1; i > -1; i--)
            {
                newArr.Add(Convert.ToInt32(firstNum.Substring(i, 1)));//前15或18位倒序存进数组
            }
            List<int> arrJiShu = new List<int>();//奇数位*2的积 <9
            List<int> arrJiShu2 = new List<int>();//奇数位*2的积 >9
            List<int> arrOuShu = new List<int>();//偶数位数组
            ...
            sumTotal = sumJiShu + sumOuShu + sumJiShuChild1 + sumJiShuChild2;//计算总和  
            int k = sumTotal % 10 == 0 ? 10 : sumTotal % 10;
            int luhm = 10 - k;//计算Luhm值
            return luhm;
        }

        public static string CreditRandomBankCard(int cnum)
        {
            string bankbin = BankBin[pub.ran.Next(0, BankBin.Length)].ToString();
            return CreditRandomBankCard(bankbin, cnum);
        }

        public static string CreditRandomBankCard(string bankbin, int cnum)
        {
            StringBuilder banknum = new StringBuilder(bankbin);
            int minnum = banknum.Length + 1;
            if (cnum < minnum)
            {
                cnum = minnum;
            }
            int fint = cnum - banknum.Length -1;
            for (int i = 0; i < fint; i++)
            {
                banknum.Append(AppPub.pub.ran.Next(0, 9));
            }
            int luhm = GetNumLuhm(banknum.ToString());
            banknum.Append(luhm.ToString());
            return banknum.ToString();
        }
    }
}